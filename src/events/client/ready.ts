...
export let ready: Event = {
	...
	execute: async (argclient: Client<true>, client: customClient) => {
		...
		// Client scheduler
		const statusTask = new Task('refreshStatus', () => refreshStatus(client), (e) => {console.log(e)})
		let statusJob = new SimpleIntervalJob({ minutes: 10, runImmediately: true}, statusTask)
