  ...
  execute: async function (message: Message, client: customClient, args: any[]) {
    ...
    if (count && !isNaN(count) && count > 0 && count <= 100) {
      ...
      if (messages && message.channel.type !== 'DM' && message.guild !== null) {
        const logChannelID = await client.servers.fetchLogChannel(message.guild);
        let logChannel;
        let reason = '';
        ...
        if (logChannel && logChannel.type === 'GUILD_TEXT') {
          const bulkDeleteEmbed = new MessageEmbed()
            .setDescription(`Bulk deletion in <#${message.channel.id}> by <@${message.author.id}>\n` + reason + Formatters.codeBlock('diff', `- Deleted ${messages.size} messages!`))
            .setColor('BLURPLE')
          ...
          return await message.channel.send({
            content: `Bulk deletion of ${messages.size} ${messages.size > 0 ? 'message' : 'messages'} by <@${message.author.id}>` + `${reason.length > 0 ? `. Reason - ${userReason}` : ''}`
          });
        }
      }
    } else {
