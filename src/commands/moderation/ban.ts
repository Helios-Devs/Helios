...
async function banMember(member: User, guild: Guild, ban: { delete: number; reason: string }) {
  const appeal = new MessageButton().setStyle('PRIMARY').setLabel('APPEAL').setCustomId(`ba-${guild.id}`);
  const appealRow = new MessageActionRow().addComponents([appeal]);
  ...
  try {
    ...
    return {
      content: `Successfully banned: ` + Formatters.inlineCode(member.tag)
    };
  } catch (e: any) {
    if (e?.message === 'Missing Permissions') {
      return {
        content: `Failed to ban: ${Formatters.inlineCode(member.tag)} due to a lack of permissions!`
      };
    } else {
...
export const ban: Command = {
  ...
  data: new SlashCommandBuilder()
    .setName('ban')
    .setDescription('Bans someone from your guild!')
  ...
  slashExecute: async function (interaction: CommandInteraction, client: customClient) {
    ...
    const days = interaction.options.getNumber('delete_messages') ?? 0;
